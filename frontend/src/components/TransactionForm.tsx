...
export const TransactionForm: React.FC<TransactionFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (transactionType === 'bank') {
        const bankTransaction: CreateBankTransactionRequest = {
          id: formData.id,
        ...
        await createBankTransaction.mutateAsync(bankTransaction);
      } else {
        const creditTransaction: CreateCreditTransactionRequest = {
          id: formData.id,
          description: formData.description,
          amount,
          date: formData.date,
        ...
        await createCreditTransaction.mutateAsync(creditTransaction);
      }
  ...
  const isLoading = createBankTransaction.isPending || createCreditTransaction.isPending;
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 flex items-center space-x-2">
            ...
            <span>
              Nova Transação {transactionType === 'bank' ? 'Bancária' : 'de Crédito'}
            </span>
          </h3>
          <button
            onClick={onClose}
